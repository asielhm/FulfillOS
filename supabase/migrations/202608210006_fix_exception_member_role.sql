-- Fix exception actions incorrectly comparing PostgreSQL CURRENT_ROLE instead
-- of the FulfillOS organization membership role. All authorization, lifecycle,
-- and append-only activity checks remain unchanged.
...
begin
  ...
  if member_role not in ('owner', 'admin', 'manager', 'operator', 'employee') then
