-- Organization service-rate catalog.
-- This complements customer-specific agreements without changing existing billing history.

create table if not exists public.organization_service_rates (
  ...
  constraint organization_service_rates_service_code_check
    check (service_code ~ '^[a-z][a-z0-9_]{1,79}$'),
  constraint organization_service_rates_service_name_check
    check (char_length(trim(service_name)) between 2 and 120),
  constraint organization_service_rates_category_check
    check (category ~ '^[a-z][a-z0-9_]{1,79}$'),
  constraint organization_service_rates_unit_check
    check (unit ~ '^[a-z][a-z0-9_]{0,39}$'),
  constraint organization_service_rates_pricing_model_check
    check (pricing_model in ('flat', 'volume_tier', 'monthly_base')),
  constraint organization_service_rates_price_check
    check (unit_price >= 0 and unit_price <= 1000000),
  constraint organization_service_rates_currency_check
    check (currency_code ~ '^[A-Z]{3}$'),
  constraint organization_service_rates_quantities_check
    check (
  ...
  constraint organization_service_rates_effective_range_check
    check (effective_to is null or effective_to > effective_from)
);

create unique index if not exists organization_service_rates_active_unique
...
create index if not exists organization_service_rates_org_active_idx
  on public.organization_service_rates (organization_id, category, service_code, effective_from desc)
  where effective_to is null;

alter table public.organization_service_rates enable row level security;

drop policy if exists "Billing roles view organization service rates"
  on public.organization_service_rates;

create policy "Billing roles view organization service rates"
  on public.organization_service_rates
  for select
  to authenticated
  using (
...
revoke all on table public.organization_service_rates
  from public, anon, authenticated;
grant select on table public.organization_service_rates to authenticated;
...
begin
  ...
  loop
    ...
    where rate.organization_id = current_organization_id
      ...
      and rate.pricing_model = normalized_pricing_model
      and rate.minimum_quantity is not distinct from normalized_minimum
      and rate.maximum_quantity is not distinct from normalized_maximum
  ...
  insert into public.audit_events (
    ...
    jsonb_build_object('saved_count', saved_count, 'unchanged_count', unchanged_count)
  );
...
revoke all on function public.configure_organization_service_rates(jsonb)
  from public, anon;
grant execute on function public.configure_organization_service_rates(jsonb)
  to authenticated;
